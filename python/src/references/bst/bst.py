...
class BST:
    ...
    def __str__(self) -> str:
        return (
            f"Node(value={self.value}, left={str(self.left)}, right={str(self.right)})"
        )
...
def fold(node: t, f: Callable[[int, b, b], b], acc: b) -> b:
    return (
        acc
        if node is None
        else f(node.value, fold(node.left, f, acc), fold(node.right, f, acc))
    )
...
def search_wparent(node: t, parent: t, v: int) -> tuple[t, t]:
    ...
    return (
        search_wparent(node.left, node, v)
        if v < node.value

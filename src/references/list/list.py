...
def map_aux(
    lst: List[a] | None, f: Callable[[a], b], acc: List[b] | None
) -> List[b] | None:
    return (
        acc
        if lst is None
        else map_aux(lst=lst.next, f=f, acc=List(value=f(lst.value), next=acc))
    )


def mapi_aux(
    lst: List[a] | None, f: Callable[[a, int], b], idx: int, acc: List[b] | None
) -> List[b] | None:
    return (
        acc
        if lst is None
        else mapi_aux(
            lst=lst.next, f=f, idx=idx + 1, acc=List(value=f(lst.value, idx), next=acc)
        )
    )
...
def filter_aux(
    lst: List[a] | None, f: Callable[[a], bool], acc: List[a] | None
) -> List[a] | None:
    if lst is None:
        return acc
    return (
        filter_aux(lst.next, f, List(value=lst.value, next=acc))
        if f(lst.value)
        else filter_aux(lst.next, f, acc)
    )


def filteri_aux(
    lst: List[a] | None, f: Callable[[a, int], bool], idx: int, acc: List[a] | None
) -> List[a] | None:
    return (
        acc
        if lst is None
...
def append_aux(left: L[a], right: L[a], acc: L[a]) -> L[a]:
    match left, right:
        ...
        case None, b:
            ...
            return append_aux(
                left=None, right=b.next, acc=List(value=b.value, next=acc)
            )
        case a, None:
            assert a is not None
            return append_aux(
                left=a.next, right=None, acc=List(value=a.value, next=acc)
            )
        case a, b:
            assert a is not None
